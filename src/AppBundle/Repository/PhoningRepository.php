...
class PhoningRepository extends EntityRepository
{
    ...
    $em = $this->_em;
    $RAW_QUERY =($region!=null) ?'select p.issu, count(*) as nombre from (select *, p.issu from (select p.client_id,p.user_id, max(p.date) as date, max(p.heure) as heure, count(p.id) as nombre from phoning p where p.date>=:startDate and p.date<=:endDate group by p.client_id,p.user_id)lp join phoning p on (lp.client_id=p.client_id and lp.date=p.date and lp.heure=p.heure)  join client c on (lp.client_id=c.id and c.ville=:region) join user_account u on (lp.user_id=u.id)) p group by p.issu ;':'select p.issu, count(*) as nombre from (select *,p.issu from (select p.client_id,p.user_id, max(p.date) as date, max(p.heure) as heure, count(p.id) as nombre from phoning p where p.date>=:startDate and p.date<=:endDate group by p.client_id,p.user_id)lp join phoning p on (lp.client_id=p.client_id and lp.date=p.date and lp.heure=p.heure)  join client c on (lp.client_id=c.id) join user_account u on (lp.user_id=u.id)) p group by p.issu ;';
